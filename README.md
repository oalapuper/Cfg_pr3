# Практическая работа №3
## Этап: 1
### ЦЕЛЬ:

создать CLI-приложение ассемблера. Реализовать разбор текстового
представления команд и трансляцию в промежуточное представление.
#

### ЗАДАНИЕ:
    1. Ассемблер должен принимать на вход аргументы командной строки:
    – Путь к исходному файлу с текстом программы.
    – Путь к двоичному файлу-результату.
    – Режим тестирования.
    2. Спроектировать человекочитаемый язык ассемблера, используя формат
    CSV. Поддержать все команды спецификации УВМ.
    3. Описать в документации (например, в README.md) спроектированный язык
    ассемблера.
    4. Реализовать транслятор, который язык ассемблера преобразует во
    внутреннее представление (например, список кортежей, объектов или
    словарей).
    5. (только для данного этапа) В режиме тестирования вывести на экран
    внутреннее представление ассемблированной программы в формате полей и
    значений, как в тесте из спецификации УВМ.
    6. Создать программу для тестов, приведенных в спецификации УВМ.
    Продемонстрировать, что ассемблер генерирует идентичные
    последовательности полей и их значений.
    7. Результат выполнения этапа сохранить в репозиторий стандартно
    оформленным коммитом.
#

### Спецификация УВМ
    Спецификация УВМ
    Загрузка константы

    _______________________
    |     A    | B        |
    |=====================|
    | Биты 0—7 | Биты 8—28|
    |=====================|
    |  231     | Константа|
    -----------------------


    Размер команды: 4 байт. Операнд: поле B. Результат: новый элемент на стеке.
    
    Тест (A=231, B=147): 0xE7, 0x93, 0x00, 0x00
    
    Чтение значения из памяти

    _______________________
    | A        | B        |
    |==========|==========|
    | Биты 0—7 | Биты 8—20|
    |==========|==========|
    | 61       | Смещение |    
    -----------------------
    
    
    Размер команды: 3 байт. Операнд: значение в памяти по адресу, которым
    является сумма адреса (элемент, снятый с вершины стека) и смещения (поле B).
    Результат: новый элемент на стеке.

    Тест (A=61, B=95): 0x3D, 0x5F, 0x00
    
    Запись значения в память

    _______________________
    | A        | B        |
    |==========|==========|
    | Биты 0—7 | Биты 8—20|
    |==========|==========|
    |  125     | Адрес    |    
    -----------------------
    
    
    Размер команды: 3 байт. Операнд: элемент, снятый с вершины стека.
    Результат: значение в памяти по адресу, которым является поле B.

    Тест (A=125, B=242): 0x7D, 0xF2, 0x00

    Бинарная операция: побитовый циклический сдвиг вправо

    _______________________
    | A                   |
    |=====================|
    | Биты 0—7            |
    |=====================|
    |  145                |    
    -----------------------
    
    
    Размер команды: 1 байт. Первый операнд: элемент, снятый с вершины стека.
    Второй операнд: элемент, снятый с вершины стека. Результат: новый элемент на
    стеке.
    Тест (A=145): 0x91

  
  

## Описание

Ассемблер преобразует текстовое представление программ в бинарный формат, понятный Учебной Виртуальной Машине.

## Формат команд

Команды записываются в CSV-формате (через запятую):

опкод,операнд

## Поддерживаемые команды
1. LOAD_CONST - Загрузка константы на стек
text

    LOAD_CONST,значение
        Код операции: 231
        Размер: 4 байта
        Биты 0-7: 231
        Биты 8-28: константа (21 бит)
        Пример: LOAD_CONST,147

2. MEM_READ - Чтение значения из памяти
text

    MEM_READ,смещение
        Код операции: 61
        Размер: 3 байта
        Биты 0-7: 61
        Биты 8-20: смещение (13 бит)
        Пример: MEM_READ,95

3. MEM_WRITE - Запись значения в память
text

    MEM_WRITE,адрес
        Код операции: 125
        Размер: 3 байта
        Биты 0-7: 125
        Биты 8-20: адрес (13 бит)
        Пример: MEM_WRITE,242

4. ROR - Побитовый циклический сдвиг вправо


        Код операции: 145
        Размер: 1 байт
        Биты 0-7: 145

## Примеры программ
Простая программа

    LOAD_CONST,100
    MEM_WRITE,0
    LOAD_CONST,200
    MEM_READ,0
    ROR,

Тестовая программа (соответствует спецификации)

## Тесты из спецификации УВМ
    LOAD_CONST,147 # → e7 00 00 93   
    MEM_READ,95    # → 3d 00 5f   
    MEM_WRITE,242  # → 7d 00 f2   
    ROR,           # → 91   
